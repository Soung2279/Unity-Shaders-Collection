...
Shader "A201-Shader/进阶功能/多功能溶解Alpha-T模式_Alpha Double-T" {
    Properties {
        ...
        _Mask_Textures ("透明度贴图(Opacity)", 2D) = "white" {}
        ...
        _Disslove_TEX ("溶解贴图(Disslove)", 2D) = "white" {}
        ...
        [MaterialToggle] _VerTexOffest ("VerTe是否启用顶点偏移xOffest", Float ) = 0
        _VertexPower ("顶点偏移数值", Vector) = (1,1,1,0)
        _VPan_U ("顶点偏移U方向流动速度", Float ) = 0
        _VPan_V ("顶点偏移V方向流动速度", Float ) = 0
        _MaskTextures ("遮罩贴图", 2D) = "white" {}
        _BackFacePower ("遮罩贴图颜色强度", Float ) = 1
        [HideInInspector]_Cutoff ("Alpha剔除数值", Range(0,1)) = 0.5
    }
    SubShader {
        Tags {
