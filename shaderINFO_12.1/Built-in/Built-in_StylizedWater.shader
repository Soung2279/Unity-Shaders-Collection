Shader "A201-Shader/Built-in/特殊制作/风格化水面"
{
    Properties
    {
        [Header(Densities)]
        _DepthDensity ("水深", Range(0.0, 1.0)) = 0.5
        _DistanceDensity ("水距离", Range(0.0, 1.0)) = 0.1
        
        [Header(Waves)]
        [NoScaleOffset]
        _WaveNormalMap ("波浪纹理(法线贴图)", 2D) = "bump"{}
        _WaveNormalScale ("缩放", float) = 10.0
        _WaveNormalSpeed ("速率", float) = 1.0

        [Header(Base Color)]
        [HDR]
        _ShallowColor ("浅水颜色", Color) = (0.44, 0.95, 0.36, 1.0)
        [HDR]
        _DeepColor ("深水颜色", Color) =  (0.0, 0.05, 0.19, 1.0)
        [HDR]
        _FarColor ("远距离颜色", Color) = (0.04, 0.27, 0.75, 1.0)
        
        [Header(Reflections)]
        _ReflectionContribution ("反射强度", Range(0.0, 1.0)) = 1.0
        ...
        [Header(Foam)]
        _FoamContribution ("焦散强度", Range(0.0, 1.0)) = 1.0
        [NoScaleOffset]
        _FoamTexture ("焦散纹理", 2D) = "black"{}
        _FoamScale ("缩放", float) = 1.0
        _FoamSpeed ("速率", float) = 1.0
        _FoamNoiseScale ("噪波强度", Range(0.0, 1.0)) = 0.5

        [Header(Sun Specular)]
        [HDR]
        _SunSpecularColor ("阳光颜色", Color) = (1, 1, 1, 1)
        _SunSpecularExponent ("混合指数", float) = 1000

        [Header(Sparkle)]
        [NoScaleOffset]
        _SparklesNormalMap ("波光纹理(法线贴图)", 2D) = "bump"{}
        _SparkleScale ("缩放", float) = 10
        _SparkleSpeed ("速率", float) = 0.75
        [HDR]
        _SparkleColor ("颜色", Color) = (1, 1, 1, 1)
        _SparkleExponent ("混合指数", float) = 10000

        [Header(Edge Foam)]
        [HDR]
        _EdgeFoamColor ("边缘泡沫颜色", Color) = (1, 1, 1, 1)
        _EdgeFoamDepth ("缩放", float) = 10.0

        [Header(Shadow Mapping)]
        [Toggle(SHADOWS)]
        _FancyShadows("启用阴影", int) = 0
        _MaxShadowDistance("最大采样距离", float) = 50.0
        _ShadowColor ("颜色", Color) = (0.5, 0.5, 0.5, 1.0)

        [Header(Vertex Waves #1)]
        _Wave1Direction ("方向", Range(0, 1)) = 0
        _Wave1Amplitude ("起伏度", float) = 1
        _Wave1Wavelength ("波浪长度", float) = 1
        _Wave1Speed ("速率", float) = 1

        [Header(Vertex Waves #2)]
        _Wave2Direction ("方向", Range(0, 1)) = 0
        _Wave2Amplitude ("起伏度", float) = 1
        _Wave2Wavelength ("波浪长度", float) = 1
        _Wave2Speed ("速率", float) = 1
    }
    SubShader
    {
        LOD 100
        ...
        Pass
        {
            ...
            #include "Built-in_StylizedWater_WaterUtilities.cginc"
            #include "Built-in_StylizedWater_ShadowUtilities.cginc"
