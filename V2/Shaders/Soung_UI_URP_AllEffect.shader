//2026.5.9 removed HSV / GamTex / Outline; added UI Rect Mask 2D support
Shader "Soung/UI/FullFx2D"
{
    Properties
    {
        ...
        // Rect Mask 2D 支持（由 Unity RectMask2D 组件自动注入）
        ...
        // Mask 组件（Stencil）支持（由 Unity Mask 组件自动注入）
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float4 uv0 : TEXCOORD1;             // UV0
                float4 custom1 : TEXCOORD2;         // Custom1.xyzw
                fixed4 color : COLOR;
                float4 custom2 : TEXCOORD3;         // Custom2.xy
                float4 worldPosition : TEXCOORD4;   // 本地坐标（用于 Mask / RectMask2D）
                float4 mask : TEXCOORD5;            // 用于 UNITY_UI_CLIP_RECT
                UNITY_VERTEX_INPUT_INSTANCE_ID
            ...
            float4 _ClipRect;
            fixed4 _TextureSampleAdd;   // Unity UI 系统注入
            ...
            v2f vert(Attributes v)
            {
                ...
                OUT.uv0.xy = v.texcoord.xy;
                OUT.uv0.zw = 0;
                OUT.custom1 = v.texcoord1;
                OUT.custom2 = v.texcoord2;
                OUT.color = v.color;

                float4 vPosition = UnityObjectToClipPos(v.vertex);
                OUT.vertex = vPosition;
                OUT.clipPosV = vPosition;
                OUT.worldPosition = v.vertex;   // Canvas 本地坐标即 RectMask2D 所需坐标

                // 计算 RectMask2D 软边 mask（与 UnityUI 标准一致）
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // alpha 精度修正（防止 HDR 混合时数值不稳定，与 UnityUI 标准一致）
                ...
                float lerpResult60 = ValueZero;
                    ...
                    float2 uv_NoiseTex = IN.uv0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
                ...
                float2 finalUV = (float2(( _MainTexPolarSets.z * ( length( temp_output_34_0_g3 ) * 2.0 ) ) , ( ( atan2( break39_g3.x , break39_g3.y ) * ( 1.0 / UNITY_TWO_PI ) ) * _MainTexPolarSets.w )));
                #elif defined(_MAINTEXUVMODE_POLARDISTORTION)
                // 极坐标扭曲模式
                float2 remappedUV = uv_MainTex * 2.0 - 1.0;
                float remappedDist = length(remappedUV);
                float rotAngle = ((1.0 - remappedDist) * 2.0 * _MainTexPolarDistortionPower) * UNITY_PI;
                ...
                float polarTheta = (atan2(polarDistortionUV.y, polarDistortionUV.x) / (2.0 * UNITY_PI)) + 0.5;
                ...
                float4 texCoord8 = IN.custom1;
                texCoord8.xy = IN.custom1.xy * float2( 1,1 ) + float2( 0,0 );
                ...
                float2 uv_DissolveTex = IN.uv0.xy * _DissolveTex_ST.xy + _DissolveTex_ST.zw;
                ...
                float4 texCoord384 = IN.custom2;
                texCoord384.xy = IN.custom2.xy * float2( 1,1 ) + float2( 0,0 );
                ...
                float2 uv_DissolveTexPlus = IN.uv0.xy * _DissolveTexPlus_ST.xy + _DissolveTexPlus_ST.zw;
                ...
                float2 texCoord406 = IN.uv0.xy * float2( 1,1 ) + float2( 0,0 );
                ...
                float4 baseColor = ( MainTexColor113 * IN.color );
                float4 dissolveBlendedColor = lerp(
                    ( baseColor * float4( DissolveColor304 , 0.0 ) ),
                    ( baseColor + dissolvealphaEDGE),
                    _DissolveColorMode
                );
                ...
                float4 tex2DNode196 = float4(0,0,0,0);
                    ...
                    float2 uv_LiuguangTex = IN.uv0.xy * _LiuguangTex_ST.xy + _LiuguangTex_ST.zw;
                    ...
                    float2 appendResult50_g4 = (float2(( _LiuguangPolarScale.z * ( length( temp_output_34_0_g4 ) * 2.0 ) ) , ( ( atan2( break39_g4.x , break39_g4.y ) * ( 1.0 / UNITY_TWO_PI ) ) * _LiuguangPolarScale.w )));
                ...
                float2 uv_MaskTex = IN.uv0.xy * _MaskTex_ST.xy + _MaskTex_ST.zw;
                ...
                float2 uv_MaskTexPlus = IN.uv0.xy * _MaskTexPlus_ST.xy + _MaskTexPlus_ST.zw;
                ...
                float Alpha = saturate( MainTexAlpha114 * IN.color.a * DissolveAlpha305 * MaskTexAlpha193 * MaskTexPlusAlpha194 );

                half4 color = half4(Color, Alpha);

                // RectMask2D 裁剪（与 UnityUI 标准 UNITY_UI_CLIP_RECT 一致）
                ...
                // 预乘 Alpha（Blend One OneMinusSrcAlpha 要求）
                color.rgb *= color.a;
