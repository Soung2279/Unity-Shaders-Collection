Shader "Soung/UI/UILiuguang"
{
    Properties
    {
        ...
        _LGAlpha("流光透明度", Range(0, 1)) = 1
        [Enum(Add,0,Multiply,1)]_LGBlendMode("流光混合模式", Float) = 0
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            uniform float _LGAlpha;
            uniform float _LGBlendMode;
            ...
            fixed4 frag(v2f IN ) : SV_Target
            {
                ...
                float4 tex2DNode7 = tex2D(_MainTex, IN.texcoord.xy);
                float lerpResult11 = lerp(tex2DNode7.r, tex2DNode7.a, _SwitchP);

                float3 mainRGB = (IN.color * tex2DNode7).rgb;
                float mainAlpha = IN.color.a * lerpResult11;

                float2 uv_LiuGuangTex = IN.texcoord.xy * _LiuGuangTex_ST.xy + _LiuGuangTex_ST.zw;
                float2 panner5 = 1.0 * _Time.y * _LGSpeed + uv_LiuGuangTex;
                ...
                float lgMask = tex2D(_LiuGuangTex, panner5).r * tex2D(_MaskTex, rotator37).r * _LGAlpha * lerpResult11;
                float3 lgContrib = _LGColor.rgb * lgMask;

                float3 addBlend = mainRGB + lgContrib;
                float3 mulBlend = lerp(mainRGB, mainRGB * _LGColor.rgb, lgMask);
                float3 finalRGB = lerp(addBlend, mulBlend, _LGBlendMode);

                half4 color = half4(finalRGB, mainAlpha);
