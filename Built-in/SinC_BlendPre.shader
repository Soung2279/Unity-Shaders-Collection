...
Shader "A201-Shader/进阶功能/深度消隐软溶解与多重遮罩_SinCBlend(优化版)"
{
	Properties
	{
		[Header(Base)]
		...
		_Indensity("基础色强度", Float) = 1
		_MainU("主贴图U方向速率", Float) = 0
		_MainV("主贴图V方向速率", Float) = 0
		_MainPower("整体颜色强度", Float) = 1
		_Opacity("透明度", Float) = 1
		_OpacityPower("透明度强度", Float) = 1
		[Header(DepthFade)]
		[Toggle]_DepthFade("是否开启深度消隐", Float) = 0
		...
		[Toggle]_DistortionInfluenceSoft("扭曲是否影响软溶解", Float) = 0
		[Header(Dissove)]
		[Toggle]_SoftDissolveSwitch("是否启用软溶解", Float) = 0
		_SoftDissolveTex("软溶解贴图", 2D) = "white" {}
		_SoftDissolveIndensity("软溶解强度", Range( 0 , 1.05)) = 0
		_SoftDissolveSoft("软溶解平滑度", Float) = 0
		_SoftDissolveTexU("软溶解贴图U方向速率", Float) = 0
		_SoftDissolveTexV("软溶解贴图V方向速率", Float) = 0
		[Toggle]_VertexColorInfluenceSoftDissolve("顶点/粒子颜色是否影响软溶解", Float) = 0
		[Toggle]_CustomDataUV2XInfluenceSoftDissolve("是否CustomDataUV2控制软溶解", Float) = 0
		...
		[Toggle]_AlphaTexUV2("Alpha贴图UV切换", Float) = 0
	...
	SubShader
	{
		...
		uniform float _DistortionInfluenceGradient;
		...
		uniform float _DistortionInfluenceSoft;
		...
		uniform float _VertexColorInfluenceSoftDissolve;
		uniform float _CustomDataUV2XInfluenceSoftDissolve;
		...
		uniform float _SoftDissolveSwitch;
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float3 desaturateInitialColor43 = tex2Dlod( _DistortionTex, float4( ( (( _Distortion2UV )?( uv1_DistortionTex ):( uv0_DistortionTex )) + ( appendResult33 * _Time.y ) ), 0, 0.0) ).rgb;
			...
			v.vertex.xyz += ( _VertexOffsetIndensity * ( tex2Dlod( _VertexOffsetTex, float4( (( _DistortionInfluenceOffset )?( lerpResult172 ):( float3( temp_output_169_0 ,  0.0 ) )).xy, 0, 0.0) ) * float4( ase_vertexNormal , 0.0 ) ) * v.color.a ).rgb;
		}
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float2 appendResult33 = (float2(_DistortionU , _DistortionV));
			float3 desaturateInitialColor43 = tex2D( _DistortionTex, ( (( _Distortion2UV )?( uv1_DistortionTex ):( uv0_DistortionTex )) + ( appendResult33 * _Time.y ) ) ).rgb;
			float desaturateDot43 = dot( desaturateInitialColor43, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar43 = lerp( desaturateInitialColor43, desaturateDot43.xxx, 1.0 );
			float3 DistortionUV49 = desaturateVar43;
			float DistortionIndeisty48 = _DistortionIndensity;
			...
			float3 desaturateInitialColor139 = ( tex2D( _AlphaTex, ( ( appendResult82 * _Time.y ) + (( _AlphaTexUV2 )?( uv1_AlphaTex ):( uv0_AlphaTex )) ) ) * tex2D( _AlphaTex2, ( ( appendResult84 * _Time.y ) + uv0_AlphaTex2 ) ) * tex2D( _AlphaTex3, ( ( appendResult90 * _Time.y ) + uv0_AlphaTex3 ) ) * tex2D( _AlphaTex4, ( ( appendResult88 * _Time.y ) + uv0_AlphaTex4 ) ) * tex2D( _AlphaTex5, ( ( appendResult94 * _Time.y ) + uv0_AlphaTex5 ) ) ).rgb;
			...
			float temp_output_65_0 = ( i.uv_tex4coord.z * _CustomDataUV2X_Indensity );
			float clampResult115 = clamp( ( (( ( desaturateVar63 * (( _VertexColorInfluenceSoftDissolve )?( i.vertexColor.a ):( 1.0 )) ) * (( _CustomDataUV2XInfluenceSoftDissolve )?( temp_output_65_0 ):( 1.0 )) )).x + 1.0 + ( _SoftDissolveIndensity * -2.0 ) ) , 0.0 , 1.0 );
			...
			o.Emission = ( float4( ( ( (tex2D( _GradientTex, (( _DistortionInfluenceGradient )?( lerpResult165 ):( float3( temp_output_162_0 ,  0.0 ) )).xy )).rgb * pow( ( (tex2DNode144).rgb * (_Color).rgb * (i.vertexColor).rgb * temp_output_151_0 ) , temp_cast_7 ) ) * _Indensity ) , 0.0 ) + ( DissolveLine168 * _LineColor * _LineIndensity ) ).rgb;
			...
			o.Alpha = (( _DepthFade )?( temp_output_203_0 ):( temp_output_200_0 ));
		}
...
/*ASEBEGIN
Version=17800
1920;0;1920;1029;4480.771;1595.203;3.152672;True;False
...
ASEEND*/
//CHKSM=3CFCD85096A8FAB8431E723072432F8EA394BF8E